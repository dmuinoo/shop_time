...
const Checkout = ({
  ...
  useEffect(() => {
    ...
    else get_payment_total(shipping_id, "");
  }, [shipping_id, coupon]);

  const showItems = () => {
    return (
      <div>
        <h4 className="text-muted mt-3">Your cart has {total_items} item(s)</h4>
        <hr />
        {items &&
          items !== null &&
          items !== undefined &&
  ...
  const displayTotal = () => {
    ...
    if (coupon && coupon !== null && coupon !== undefined) {
      result.push(
        <Fragment>
          ...
          <div>
            <span className="text-muted mr-2">Discounted Items:</span>
            <span style={{ color: "#b12704" }}>{total_after_coupon}€</span>
          </div>
        </Fragment>
      );
    } else {
    ...
    if (shipping && shipping_id !== 0) {
      result.push(
        <div className="mt-3">
          <span className="text-muted mr-3">Shipping &amp; Handling:</span>
          <span style={{ color: "#b12704" }}>{shipping_cost}€</span>
        </div>
      );
    } else {
      result.push(
    ...
    result.push(
      <div className="mt-3">
        <span className="text-muted mr-3">Estimated HST:</span>
        <span style={{ color: "#b12704" }}>{estimated_tax}€</span>
      </div>
    );
    ...
    result.push(
      <div className="mt-3" style={{ color: "#b12704" }}>
        <span className="text-muted mr-3">Order Total:</span>
        ...
        <Fragment>
          ...
          {loading ? (
            <div className="d-flex justify-content-center align-items-center">
              <Loader type="Oval" color="#424242" height={50} widht={50} />
            </div>
          ) : (
            <button
  ...
  if (made_payment) return <Redirect to="/thankyou" />;
  ...
  return (
    <div className="container mt-5 mb-5">
      ...
      <div className="row">
        <div className="col-7">
          {showItems()}
          <h4 className="text-muted">Gift cards &amp; promotional codes</h4>
          <form
            className="mt-3"
            style={{ width: "60%" }}
        ...
        <div className="col-5">
          ...
          <div style={{ fontSize: "18px" }}>{displayTotal()}</div>
          <ShippingForm
            full_name={full_name}
